...
class Data:
    def __init__(self):
        ...
        # Networks
        self.use_char = True
        self.word_feature_extractor = "GRU" # "LSTM"/"CNN"/"GRU"/
    ...
    def build_alphabet(self, X, Y):
        ...
        print('data label_alphabet_size:', self.label_alphabet_size)
        for idx in range(self.feature_num):
    ...
    def build_pretrain_emb(self):
        if self.word_emb_dir:
            ...
            self.pretrain_word_embedding, self.word_emb_dim = functions.build_pretrain_embedding(self.word_emb_dir, self.word_alphabet, self.word_emb_dim, self.norm_word_emb)
        if self.char_emb_dir:
            print("Load pretrained char embedding, norm: %s, dir: %s" % (self.norm_char_emb, self.char_emb_dir))
            self.pretrain_char_embedding, self.char_emb_dim = functions.build_pretrain_embedding(self.char_emb_dir, self.char_alphabet, self.char_emb_dim, self.norm_char_emb)
        for idx in range(self.feature_num):
            if self.feature_emb_dirs[idx]:
                ...
                self.pretrain_feature_embeddings[idx], self.feature_emb_dims[idx] = functions.build_pretrain_embedding(self.feature_emb_dirs[idx], self.feature_alphabets[idx], self.feature_emb_dims[idx], self.norm_feature_embs[idx])

    def generate_instance(self, X, Y, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = functions.read_instance(X, Y, self.word_alphabet, self.char_alphabet, self.feature_alphabets,
                                                                       self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = functions.read_instance(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "test":
            self.test_texts, self.test_Ids = functions.read_instance(X, Y, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = functions.read_instance(self.raw_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        else:
