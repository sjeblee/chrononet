...
class CharCNN(nn.Module):
    def __init__(self, alphabet_size, pretrain_char_embedding, embedding_dim, hidden_dim, dropout, gpu, device='cuda:2'):
        super(CharCNN, self).__init__()
        print("build char sequence feature extractor: CNN ...")
        print('charcnn alphabet size:', alphabet_size)
        self.gpu = gpu
        self.device = device
        ...
        if self.gpu:
            self.char_drop = self.char_drop.to(device)
            self.char_embeddings = self.char_embeddings.to(device)
            self.char_cnn = self.char_cnn.to(device)
    ...
    def get_last_hiddens(self, input, seq_lengths):
        ...
        print('char inputs:', input.size())
        print('char embedding dim:', self.embedding_dim, 'alphabet size:', self.alphabet_size)
