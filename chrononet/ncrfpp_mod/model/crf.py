...
class CRF(nn.Module):
    ...
    def __init__(self, tagset_size, gpu):
        ...
        init_transitions[0, :] = -10000.0
        if self.gpu:
            init_transitions = init_transitions.to(tdevice)
    ...
    def _calculate_PZ(self, feats, mask):
        ...
        print('crf batch_size:', batch_size, 'seq_len:', seq_len, 'tag_size:', tag_size)
        ...
        for idx, cur_values in seq_iter:
            #print('crf idx:', idx, cur_values.size())
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        for idx, cur_values in seq_iter:
            ...
            cur_values = cur_values + partition.contiguous().view(batch_size, tag_size, 1).expand(batch_size, tag_size, tag_size)
            partition, cur_bp = torch.max(cur_values[:, :-2, :], 1) # do not consider START_TAG/STOP_TAG
            ...
            # print cur_bp
            # print "one best, ",idx
            #print('partition:', partition, 'cur_bp:', cur_bp)
            partition_history.append(partition)
        ...
        for idx in range(len(back_points)-2, -1, -1):
            #print('- decoding idx:', idx)
            pointer = torch.gather(back_points[idx], 1, pointer.contiguous().view(batch_size, 1))
            #print('- pointer:', pointer.size())
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        new_tags = torch.zeros((batch_size, seq_len)).long()
        if self.gpu:
            new_tags = new_tags.to(tdevice)
        ...
        if self.gpu:
            length_mask = length_mask.to(tdevice)
            tags = tags.to(tdevice)
    ...
    def neg_log_likelihood_loss(self, feats, mask, tags):
        ...
        print('f:', forward_score.item(), " g:", gold_score.item(), " dis:", forward_score.item() - gold_score.item())
        # exit(0)
    ...
    def _viterbi_decode_nbest(self, feats, mask, nbest):
        ...
        for idx in range(len(back_points)-2, -1, -1):
            ...
            decode_idx[idx] = new_pointer/nbest
