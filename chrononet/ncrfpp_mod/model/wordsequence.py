...
class WordSequence(nn.Module):
    def __init__(self, data):
        ...
        if self.gpu:
            self.droplstm = self.droplstm.to(data.device)
            self.hidden2tag = self.hidden2tag.to(data.device)
            if self.word_feature_extractor == "CNN":
                self.word2cnn = self.word2cnn.to(data.device)
                for idx in range(self.cnn_layer):
                    self.cnn_list[idx] = self.cnn_list[idx].to(data.device)
                    self.cnn_drop_list[idx] = self.cnn_drop_list[idx].to(data.device)
                    self.cnn_batchnorm_list[idx] = self.cnn_batchnorm_list[idx].to(data.device)
            else:
    ...
    def forward(self, word_inputs, char_inputs=None, char_seq_lengths=None):
        ...
        word_represent = self.wordrep(word_inputs, char_inputs=char_inputs, char_seq_lengths=char_seq_lengths)
        if self.word_feature_extractor == "CNN":
            ...
            packed_words = word_represent # For batch size of 1
            hidden = None
            lstm_out, hidden = self.lstm(packed_words, hidden)
            #lstm_out, _ = pad_packed_sequence(lstm_out)
            # Lstm_out (seq_len, seq_len, hidden_size)
            #feature_out = self.droplstm(lstm_out) #.transpose(1, 0))
            feature_out = lstm_out
